        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            @forelse($medias as $media)
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-zinc-100 overflow-hidden group hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                    ...
                    <div class="aspect-[4/5] relative overflow-hidden bg-zinc-100">
                        ...
                        @else
                            <div class="absolute inset-0 flex items-center justify-center bg-black/40">
                                <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center text-theatre-red">▶️</div>
                            </div>
                        ...
                        <div class="absolute top-4 left-4 z-20">
                            <span class="px-3 py-1 bg-amber-500 text-white rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">
                                Support Pédagogique
                            </span>
                        </div>
                    </div>
                    ...
                    <!-- Content -->
                    <div class="p-8">
                        <h3 class="text-2xl font-serif font-bold text-zinc-900 mb-3">{{ $media->title }}</h3>
                        <p class="text-zinc-500 text-sm leading-relaxed mb-8">
                            {{ $media->description ?? 'Ressource de formation pour les ateliers théâtre.' }}
                        </p>
                        
                        <div class="flex items-center justify-between">
                            <div class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Posté par : {{ $media->user ? $media->user->name : 'Le Modérateur' }}</div>
                            <a href="{{ $media->type === 'video' ? $media->file_path : asset('storage/' . $media->file_path) }}" 
                               ...
                               class="bg-zinc-900 text-white px-6 py-2.5 rounded-xl text-xs font-bold hover:bg-black transition-all shadow-md">
                                {{ $media->type === 'document' ? 'Télécharger' : 'Regarder' }}
                            </a>
                        </div>
                    </div>
                </div>
