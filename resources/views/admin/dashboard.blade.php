...
<div class="max-w-7xl mx-auto space-y-8">
    ...
    <div id="media-management" class="bg-white rounded-3xl shadow-sm border border-zinc-100 overflow-hidden" x-data="{ showMediaModal: false, mediaType: 'photo', videoUrls: [''] }">
        <div class="p-6 border-b border-zinc-100 flex justify-between items-center">
            <div>
                ...
                <p class="text-zinc-500 text-sm">Gérez les contenus (photos, vidéos, documents).</p>
            </div>
            <button @click="showMediaModal = true" class="bg-theatre-red text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-red-700 transition-all">+ Ajouter un média</button>
        </div>
        ...
        <div x-show="showMediaModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-xl relative z-10 overflow-hidden">
                ...
                <form action="{{ route('admin.medias.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                    @csrf
                    <div>
